...
unreservedWord_
    ...
    | IMPORT | CONCURRENT | XML | POSITION | SHARE | DUMPFILE | BOTTOM
    ;


tableName
    : ((dbName DOT_) (rpName DOT_) | (rpName DOT_))? name
    ;
...
columnName
    : name (DOT_ columnName)? //(owner DOT_)? name
    ;
...
functionCall
    : aggregationFunction | selectorFunction_ | transformationFunction_ | regularFunction_
    ;

aggregationFunction
    : aggregationFunctionName_ LP_ distinct? (expr (COMMA_ expr)* | ASTERISK_ | ASTERISK_ (COMMA_ expr))? RP_ overClause_?
    ;
...
selectorFunction_
    : selectorFunctionName_ LP_ (expr (COMMA_ expr)* | ASTERISK_)? RP_
    ;
...
transformationFunction_
    : transformationFunctionName_ LP_ (expr (COMMA_ expr)* | ASTERISK_ | ASTERISK_ (COMMA_ expr))? RP_
    ;
