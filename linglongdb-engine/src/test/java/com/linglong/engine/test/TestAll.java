...
import com.linglong.engine.core.lock.LockTimeoutException;
...
public class TestAll {
    ...
    public void existsLockWait() throws Exception {
        ...
        Thread t = TestUtils.startAndWaitUntilBlocked(new Thread(() -> {
            try {
    ...
    private void testFill(final View ix, final int count) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            byte[] key = TestUtils.randomStr(rnd, 1, 100);
            byte[] value = TestUtils.randomStr(rnd, 1, 100);
