...
public class Locker extends LockOwner {
    ...
    public final LockResult tryLockShared(long indexId, byte[] key, long nanosTimeout)
            throws DeadlockException {
        return tryLock(LockManager.TYPE_SHARED, indexId, key, LockManager.hash(indexId, key), nanosTimeout);
    }

    final LockResult tryLockShared(long indexId, byte[] key, int hash, long nanosTimeout)
            throws DeadlockException {
        return tryLock(LockManager.TYPE_SHARED, indexId, key, hash, nanosTimeout);
    }
    ...
    final LockResult lockSharedNT(long indexId, byte[] key, int hash, long nanosTimeout)
            throws LockFailureException {
        return lockNT(LockManager.TYPE_SHARED, indexId, key, hash, nanosTimeout);
    }

    public final LockResult tryLockUpgradable(long indexId, byte[] key, long nanosTimeout)
            throws DeadlockException {
        return tryLock(LockManager.TYPE_UPGRADABLE, indexId, key, LockManager.hash(indexId, key), nanosTimeout);
    }

    final LockResult tryLockUpgradable(long indexId, byte[] key, int hash, long nanosTimeout)
            throws DeadlockException {
        return tryLock(LockManager.TYPE_UPGRADABLE, indexId, key, hash, nanosTimeout);
    }
    ...
    final LockResult lockUpgradableNT(long indexId, byte[] key, int hash, long nanosTimeout)
            throws LockFailureException {
        return lockNT(LockManager.TYPE_UPGRADABLE, indexId, key, hash, nanosTimeout);
    }

    public final LockResult tryLockExclusive(long indexId, byte[] key, long nanosTimeout)
            throws DeadlockException {
        return tryLock(LockManager.TYPE_EXCLUSIVE, indexId, key, LockManager.hash(indexId, key), nanosTimeout);
    }

    final LockResult tryLockExclusive(long indexId, byte[] key, int hash, long nanosTimeout)
            throws DeadlockException {
        return tryLock(LockManager.TYPE_EXCLUSIVE, indexId, key, hash, nanosTimeout);
    }
    ...
    public final LockResult lockExclusive(Lock lock, long nanosTimeout) throws LockFailureException {
        ...
        throw failed(LockManager.TYPE_EXCLUSIVE, result, nanosTimeout);
    }

    final LockResult lockExclusiveNT(long indexId, byte[] key, int hash, long nanosTimeout)
            throws LockFailureException {
        return lockNT(LockManager.TYPE_EXCLUSIVE, indexId, key, hash, nanosTimeout);
    }
    ...
    public final Lock lockSharedNoPush(long indexId, byte[] key) throws LockFailureException {
        ...
        if (!result.isHeld()) {
            throw failed(LockManager.TYPE_SHARED, result, -1);
        }
    ...
    public final Lock lockUpgradableNoPush(long indexId, byte[] key) throws LockFailureException {
        ...
        if (!result.isHeld()) {
            throw failed(LockManager.TYPE_UPGRADABLE, result, -1);
        }
    ...
    public final LockResult lockCheck(long indexId, byte[] key) {
        return manager().check(this, indexId, key, LockManager.hash(indexId, key));
    }
    ...
    protected final ParentScope scopeEnter() {
    ...
    protected final PendingTxn transferExclusive() {
        PendingTxn pending;
    ...
    public static final class ParentScope {
        public ParentScope mParentScope;
        public Object mTailBlock;
        public int mTailBlockSize;
        public LockMode mLockMode;
        public long mLockTimeoutNanos;
        public int mHasState;
        public long mSavepoint;
    }
}
