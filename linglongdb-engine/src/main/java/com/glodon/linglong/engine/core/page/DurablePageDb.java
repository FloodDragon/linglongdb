    ...
    */
    ...
    private DurablePageDb(final EventListener debugListener,
        ...
        try {
            ...
            if (destroy || mPageArray.isEmpty()) {
                ...
                /*P*/
                byte[] header = PageOps.p_calloc(pageSize, isDirectIO());
                try {
                ...
                try {
                    ...
                    findHeader:
                    {
                        ...
                        try {
                            ...
                            commitNumber0 = PageOps.p_intGetLE(header0, I_COMMIT_NUMBER);
                            pageSize0 = PageOps.p_intGetLE(header0, I_PAGE_SIZE);
                            ex0 = null;
                        ...
                        try {
                            ...
                            commitNumber1 = PageOps.p_intGetLE(header1, I_COMMIT_NUMBER);
                        } catch (IncompleteRestoreException e) {
                            throw e;
                        } catch (CorruptDatabaseException e) {
                            if (ex0 != null) {
                        ...
                        int pageSize1 = PageOps.p_intGetLE(header1, I_PAGE_SIZE);
    ...
    Snapshot beginSnapshot(LocalDatabase db) throws IOException {
        ...
        try {
            ...
            /*P*/
            byte[] header = PageOps.p_alloc(MINIMUM_PAGE_SIZE, isDirectIO());
            try {
    ...
    public static PageDb restoreFromSnapshot(int pageSize, File[] files, FileFactory factory,
        ...
        try {
            ...
            checkMagicNumber(Utils.decodeLongLE(buffer, I_MAGIC_NUMBER));
            pageSize = Utils.decodeIntLE(buffer, I_PAGE_SIZE);
    ...
    public static PageDb restoreFromSnapshot(PageArray pa, PageCache cache, Crypto crypto, InputStream in)
        ...
        try {
            ...
            checkMagicNumber(Utils.decodeLongLE(buffer, I_MAGIC_NUMBER));
            int pageSize = Utils.decodeIntLE(buffer, I_PAGE_SIZE);
    ...
    private static PageDb restoreFromSnapshot(PageCache cache, Crypto crypto, InputStream in,
        ...
        // complete.
        Utils.encodeLongLE(buffer, I_MAGIC_NUMBER, INCOMPLETE_RESTORE);
        ...
        /*P*/
        byte[] bufferPage = PageOps.p_transfer(buffer, pa.isDirectIO());
        ...
        try {
            ...
            try {
                ...
                PageOps.p_longPutLE(bufferPage, I_MAGIC_NUMBER, MAGIC_NUMBER);
    ...
    private void commitHeader(final /*P*/ byte[] header, final int commitNumber)
        ...
        PageOps.p_longPutLE(header, I_MAGIC_NUMBER, MAGIC_NUMBER);
        PageOps.p_intPutLE(header, I_PAGE_SIZE, array.pageSize());
        PageOps.p_intPutLE(header, I_COMMIT_NUMBER, commitNumber);
        ...
        for (int i = 1; i < dupCount; i++) {
            PageOps.p_copy(header, 0, header, i * MINIMUM_PAGE_SIZE, MINIMUM_PAGE_SIZE);
        }
    ...
    private static int setHeaderChecksum(/*P*/ byte[] header) {
        ...
        PageOps.p_intPutLE(header, I_CHECKSUM, 0);
        int checksum = PageOps.p_crc32(header, 0, MINIMUM_PAGE_SIZE);
        PageOps.p_intPutLE(header, I_CHECKSUM, checksum);
        return checksum;
    }
    ...
    private /*P*/ byte[] readHeader(int id) throws IOException {
        ...
        try {
            ...
            checkMagicNumber(PageOps.p_longGetLE(header, I_MAGIC_NUMBER));

            int checksum = PageOps.p_intGetLE(header, I_CHECKSUM);
    ...
    private void readPartial(long index, int start, byte[] buf, int offset, int length)
        ...
        if (isDirectIO()) {
            readLen = pageSize();
            page = PageOps.p_alloc(readLen, true);
        } else {
            readLen = start + length;
            page = PageOps.p_alloc(readLen, false);
        }
